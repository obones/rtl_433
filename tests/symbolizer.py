...
def process_source(path, name):
    ...
    with open(join(path, name), encoding='utf-8', errors='replace') as f:
        ...
        for i, line in enumerate(f):
            ...
            # static int foo_callback(r_device *decoder, bitbuffer_t *bitbuffer, ...
            # static int foo_callback(r_device *decoder, bitrow_t bits, uint16_t bits_num_bits)
            # static int foo_callback(r_device *decoder, bitrow_t bits, uint16_t bits_num_bits, ...)
            # static int
            # foo_callback(r_device *decoder, bitbuffer_t *bitbuffer)
            m = re.match(
                r'(?:\s*static\s*int\s*)?([a-zA-Z0-9_]+)\(\s*r_device\s+\*\s*[a-z]+\s*,\s*(?:bitbuffer_t\s+\*|bitrow_t\s+\s*[a-z]+,\s*uint16_t)\s*[a-z]+', line)
            if m:
                # print(m.group(1))
                fName = m.group(1)
                fLine = i + 1
